...
use crate::configuration::{
    ...
    NUN_S3_NUMBER_OF_PARTITIONS, NUN_S3_PREFIX, NUN_S3_READ_PREFIX, NUN_S3_RETRY,
    NUN_S3_SECRET_KEY,
};
...
pub struct S3PartitionStorage {}
impl S3PartitionStorage {
            ...
            let _result = Result::or_else(
                retry(
                    || {
                        ...
                        let mut file_buffer: BytesMut =
                            BytesMut::with_capacity(OP_RECORD_SIZE * 10);
                        ...
                        let store_result = rt.block_on(S3PartitionStorage::store_buffer_to_s3(
                            file_buffer,
                            &format!("{}/{}.nun", db_name, partition),
                        ));
                        match store_result {
                            Ok(_) => {
                                log::debug!("S3PartitionStorage::store_buffer_to_s3 ok");
                                Ok::<(), String>(())
                            }
                            Err(err) => {
                                log::error!(
                                    "S3PartitionStorage::store_buffer_to_s3 failed: {}",
                                    err
                                );
                                Err::<(), String>(err)
                            }
        ...
        let dbs_threads: Vec<thread::JoinHandle<()>> = db_names
            ...
            .map(move |db_name| {
                ...
                let db_thread = thread::spawn(move || {
                    ...
                    let db = match S3PartitionStorage::read_data_from_cloud(&db_name) {
                        Ok(db) => db,
...
fn get_patirion_from_key(key: &String) -> u64 {
    S3PartitionStorage::hash(key.to_string()) % *NUN_S3_NUMBER_OF_PARTITIONS
}
...
mod tests {
    ...
    fn should_store_data_in_s3() {
        ...
        S3PartitionStorage::storage_data_on_cloud(&db, true, &final_db_name);
        let db = S3PartitionStorage::read_data_from_cloud(&final_db_name).unwrap();
    ...
    fn should_store_a_lot_data_in_s3() {
        ...
        S3PartitionStorage::storage_data_on_cloud(&db, false, &final_db_name);

        let db_after = S3PartitionStorage::read_data_from_cloud(&final_db_name).unwrap();
        ...
        let changed_keys =
            S3PartitionStorage::storage_data_on_cloud(&db_after, false, &final_db_name);

        assert!(changed_keys == 3068, "Changed keys: {}", changed_keys); // Tho only one key
                                                                         // changed we still have
                                                                         // to update +10% of the
                                                                         // data
        let db_after_update = S3PartitionStorage::read_data_from_cloud(&final_db_name).unwrap();
    ...
    fn should_read_all_dbs_from_s3() {
        ...
        S3PartitionStorage::storage_data_on_cloud(&db, true, &db_name);
        S3PartitionStorage::storage_data_on_cloud(&db1, true, &db1_name);
        let db = S3PartitionStorage::read_data_from_cloud(&db1_name).unwrap();
        log::debug!(
            "{:?}, count, keys {:?}",
            db.count_keys(),
        ...
        let dbs = prep_env();
        S3PartitionStorage::load_all_dbs_from_cloud(&dbs);
